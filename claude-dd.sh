...
if [ "$CREATE_CONTAINER" = true ]; then
    ...
    docker exec "$CONTAINER" bash -c "
        if [ -d /root/.ssh-host ]; then
            cp -r /root/.ssh-host /home/claude-user/.ssh
            chown -R claude-user:claude-user /home/claude-user/.ssh
            chmod 700 /home/claude-user/.ssh
            find /home/claude-user/.ssh -type f -exec chmod 600 {} \;
        fi
    "
else
...
docker exec -it "$CONTAINER" su claude-user -c "claude --dangerously-skip-permissions"